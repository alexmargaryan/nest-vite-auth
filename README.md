...
- `@nest-vite-auth/ui`: a stub React component library shared by both `web` and `docs`
  applications
- `@nest-vite-auth/eslint-config`: `eslint` configurations (includes
  `eslint-config-next` and `eslint-config-prettier`)
- `@nest-vite-auth/typescript-config`: `tsconfig.json`s used throughout the monorepo
